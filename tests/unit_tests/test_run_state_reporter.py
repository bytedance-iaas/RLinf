...
def test_manifest_written_at_construction(tmp_path):
    ...
    assert manifest["heartbeat_interval_s"] == 3600
...
def test_progress_and_metric_timestamps_are_independent(reporter):
    """Heartbeat, progress, and metric timestamps advance independently.
...
def test_lifecycle_marks_stopped_on_a_successful_system_exit(reporter, code):
...
@pytest.mark.parametrize("code", [1, 2, 255, -1, "boom", 0.0])
def test_lifecycle_marks_failed_on_an_unsuccessful_system_exit(reporter, code):
    with pytest.raises(SystemExit) as raised:
        with reporter.run_lifecycle():
            raise SystemExit(code)
