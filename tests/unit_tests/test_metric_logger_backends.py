...
* **``finish`` is idempotent.** Runner teardown, lifecycle cleanup, and
  interpreter shutdown can all close the same logger.
* **An unsupported capability warns and skips.** Missing presentation features
  must not fail a training run.
...
def test_log_table_warns_and_skips_when_unsupported(metric_logger, capsys):
    """Missing table support does not fail metric logging."""
...
def test_finish_is_idempotent(metric_logger):
    """Runner teardown and fallback cleanup can safely close the same logger."""
    fake = _install(metric_logger, a=FakeBackend())["a"]
