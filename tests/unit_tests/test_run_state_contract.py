...
def test_fresh_heartbeat_with_stale_progress_is_degraded():
    """A fresh heartbeat does not hide a stalled training loop.
...
def test_async_run_reports_concurrent_components():
    """Async runs expose env, rollout, and actor activity independently.
...
def test_step_semantics_is_declared():
    """Step semantics prevent incompatible x-axes from being conflated."""
...
def test_checkpoint_entry_is_structured_not_a_command_string():
    """Resume metadata stays structured instead of embedding a shell command."""
