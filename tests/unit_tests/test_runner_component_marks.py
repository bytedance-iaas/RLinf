...
These assertions read the source rather than running the loop, because
``embodied_runner`` imports ray at module load and a driver loop needs a live
cluster. What is being checked is a property of the call sites, and call sites
are what the earlier media-index bug got wrong while the API-level tests all
passed: the tests called ``flush_video(video_sub_dir="eval")``, which no
production caller ever does. Reading the call site is the cheap way to not
repeat that.
"""
