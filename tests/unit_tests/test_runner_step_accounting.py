...
Two properties, both broken once and both invisible in the reported numbers --
which is what makes them worth pinning rather than trusting to review:

* **The step loop gains no blocking call for diagnostics.** ``_advance_env_step``
  exists to label videos, and it once called ``.wait()``, putting a synchronous
  round-trip to the whole env group into every iteration of both async runners.
  A measurement tool that blocks the loop it measures reports a slowdown it
  caused itself, and reports it as if it were the workload.

* **``step_duration_s`` covers the whole step.** The async runners reported
  progress *before* the weight sync, so the dashboard's step time and its ETA
  were short by that sync -- real per-step work, unlike the eval and save the
  synchronous runner excludes deliberately. TensorBoard's ``time/step`` covers
  the full ``timer("step")`` block, so the two disagreed, and the dashboard was
  the optimistic one.
