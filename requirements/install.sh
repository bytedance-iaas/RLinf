...
# Which GitHub accelerator to use lives in its own script, so a Docker build can
# resolve it once and hand the same answer to every install.sh invocation and to
# the Dockerfile's own clones. See requirements/github_mirror.sh for the
# resolution order and the probe.
...
setup_mirror() {
    if [ "$USE_MIRRORS" -eq 1 ]; then
        ...
        # build, and only otherwise is a mirror measured here.
