...
# GitHub mirrors considered when --use-mirrors is on. Which one is fastest moves
# around between networks and over time, and a mirror that has gone slow is the
# usual reason an install or a CI job stalls, so the prefix is measured at
# install time instead of hardcoded.
...
setup_mirror() {
    if [ "$USE_MIRRORS" -eq 1 ]; then
        export USE_MIRRORS
        # An explicit GITHUB_PREFIX wins; otherwise pick whichever mirror is
        # fastest right now.
        if [ -z "$GITHUB_PREFIX" ]; then
            GITHUB_PREFIX="$(pick_fastest_github_prefix)"
        fi
