...
**当前状态**：仿真训练与 sim2real 协同训练均已完成；**上真机前的离线门已通过**（留出集比值 0.70 < 1），部署方案见 `SIM2REAL_PLAN_ZH.md`。剩余局限见 §12。
...
## 8. 阶段 G —— 真机协同训练（打通 sim2real）

A–F 只回答了"在仿真里会不会做这件事"。**换成真实相机图像，策略还认不认得，是另一个问题**，而且答案一开始是否定的。

### 8.1 先把问题量出来（不碰机器人）

`offline_replay_check.py`：把真机录制的（前视、腕视、关节状态）喂给策略，比较它预测的动作与人类当时的实际动作。两个对照让数字可解释——**仿真数据**（分布内参照）和**"完全不动"**（动作幅度的尺度）。比值 <1 表示优于什么都不做。
...
两者**互为镜像**：各自在自己的域里好、在对方的域里差。这既证明这是**视觉域鸿沟**而非某个部件的 bug，也证明这个指标本身可信。成本 20 分钟、零硬件。

**不要停在第一个嫌疑上。** 当时的首要嫌疑是仿真腕部相机指向错误（它拍的是机械臂自己）。但**把这一路砍掉后真机比值几乎不变**（4.47 → 4.59）——差距比这一个缺陷更广。如果当时收手去修相机，**30 小时机时会白花**。

### 8.2 做法：把真机数据掺进仿真数据集重训

```bash
python tools_so101_session/convert_cotrain_heldout.py    # 仿真 1292 集 + 真机 0–69 集 ×3
python examples/sft/train_vla_sft.py \
  --config-path $PWD/examples/sft/config/ --config-name so101_sft_v15 \
  runner.logger.log_path=/data08/henryg/pai/results/so101_sft_v15
```

| 参数 | 值 | 理由 |
|---|---|---|
| 热启动 | 阶段 F 的 `global_step_30` | 血统继续，不重算 norm_stats |
| 真机上采样 | ×3 | 87 集对 1292 集，不上采样梯度里几乎看不见 |
| 学习率 | 1e-5 | 轻量微调，别把仿真能力洗掉 |
| 步数 | 2000，每 250 存 | 门评逐个检查点做 |

**真机数据必须留出一部分**。v14 把 87 集全部拿去训练，然后又用其中几集读离线指标——那个 0.84 是**训练集口径**，不能作为上机依据。v15 用 0–69 训练、**70–86 从不参训**，才是第一个诚实的数字。
...
| 轮次 | 做法 | 真机比值 | 仿真环 1 |
|---|---|---|---|
| v14 | 真机全部 87 集 ×2 | 0.84（训练集口径，**不可用**） | 63.3% |
| **v15** | 真机 0–69 ×3，70–86 留出 | **0.70（留出集）** | **62.5% / 65.6%** |

交付物 `so101_sft_v15/.../global_step_1000`。**仿真成绩没有下降**（57.8% → 约 60%，在 ±4 点的抖动内甚至略升），真机比值从 4.47 降到 0.70——**离线门通过，可以上真机**。

**耗时** 约 5.5 h 建数据集（纯 CPU）+ 约 1.5 h 训练。
...
## 12. 局限与下一步（写报告时必须一并说明）

### 12.1 已经解决的：视觉域鸿沟

阶段 F 的产物在真实观测上比"完全不动"还差 4.5 倍（比值 4.47）。阶段 G 的协同训练把它压到 **0.70**（留出集口径），**离线门已通过**。详见 §8，部署方案见 `SIM2REAL_PLAN_ZH.md`。
...
**任何改动都必须重新过 §8.1 那个离线检验，且按 §8.3 的双轴门评选点。**
