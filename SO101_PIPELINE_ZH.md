...
真机数据只有 87 集，直接训出来的策略在仿真里**成功率为零**（实测 5 次运行、约 2000 轮、零抓取）。所以流程分三段：
...
| **RL 是放大器** | PPO 放大已有的成功，不发现新成功；先决条件必须在 **rollout 分布**下测 | 起点为 0 时 5 次运行零抓取；带噪成功率 1% 时 PPO 9 轮摧毁策略 |
...
**耗时** 约 3 h。**不做仿真评测**——真机数据训出的策略在仿真里是 0，它只是阶段 B 的热启动权重。
