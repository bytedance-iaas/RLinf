...
Migration uses a **dual write**: both the canonical and legacy keys are emitted
so existing wandb and TensorBoard dashboards keep working. A deprecation
warning fires once per prefix -- not per key or per step -- because these are
logged every step and a per-call warning would bury useful output.
"""
...
def _warn_deprecated(prefix: str, canonical: str) -> None:
    ...
    try:
        get_logger().warning(
            ...
            f"'{canonical.rsplit('/', 1)[0]}/'. Both are logged for compatibility."
        )
