...
checkpoint?" Before this, those existed only in a stdout ASCII table.
...
class RunStateReporter:
    ...
    def _render_locked(self) -> dict:
        ...
        return {
            ...
            "progress": {
                "step": self._step,
                # Read from the estimator, not from `self._max_steps`, so the
                # horizon shown and the horizon the ETA divides by cannot
                # disagree. They did: with `max_epochs: 1` and `max_steps: 3` a
                # run finishing its only step rendered "step 1 of 3" next to an
                # `eta_s` of 0, since the estimator had the effective total and
                # this field had the config cap.
    ...
    def _finish_metrics(self) -> None:
        ...
        Doing this in the lifecycle's ``finally`` makes teardown structural: a
        runner added later cannot forget it, which is how
        ``AsyncEmbodiedRunner`` came to leak its writer.
        """
    ...
    def run_lifecycle(self):
        ...
        Placed around the whole loop, this is the single point where every
        runner gets ``failed`` on exception and ``stopped`` on interrupt --
        18 entry scripts call bare ``runner.run()`` with no try/finally, so
        without this there is nowhere for a terminal state to be written.
...
def build_media_index(cfg, shard: int = 0) -> MediaIndexWriter | None:
    ...
    The index is written even though video playback itself is out of scope:
    without it, every historical run would need backfilling later.
    """
    try:
        runner_cfg = cfg.runner
