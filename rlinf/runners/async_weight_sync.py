...
class AsyncWeightSyncMixin:
    ...
    def init_weight_sync_state(self) -> None:
        ...
        if self.sync_weight_no_wait:
            weight_syncer_type = self.cfg.get("weight_syncer", {}).get("type", None)
            if weight_syncer_type != "patch":
                raise ValueError(
                    "actor.sync_weight_no_wait=true requires "
                    "weight_syncer.type=patch so a rollout cannot observe a "
                    "partially applied bucket sync."
                )
    ...
    def _cleanup_pending_rollout_weight_sync(self, no_wait: bool) -> bool:
        ...
        # has applied the weights, rather than merely acknowledging the request.
