...
Why not a plain range-to-range affine map
-----------------------------------------

An affine map between the URDF joint limits and the motor limits (the approach
that works for the LeIsaac SO101 USD, see ``rlinf/envs/isaaclab/so101_utils.py``
in the IsaacLab line of work) is NOT correct here. It assumes the simulated
joint range and the real servo travel share endpoints. Two places where that
assumption breaks on the ManiSkill SO100 URDF, both measured:

* the stock URDF under-models the real arm's travel (see ``so101_agent.py``);
* the gripper is not a scaled copy of the real one at all -- the real feetech
  parallel gripper and the sim revolute jaw have different linkage geometry, so
  the tick->rad conversion leaves the jaws at an 8.1 cm gap while the target
  cube is 2.9 cm, i.e. the policy can never close on it. The gripper therefore
  gets its own measured map (see ``GRIPPER_RAD_*`` below).

NOTE: SO101_CALIB is one physical robot's follower calibration. Replace it if
the arm is recalibrated or you are using a different unit.
"""

import numpy as np

# Joint order for every SO101 array in this module, matching the dataset
# action/state column order.
SO101_JOINT_NAMES = (
    "shoulder_pan",
    "shoulder_lift",
    "elbow_flex",
...
def _check_width(array, name):
    """Fail loudly on a wrong-width array instead of broadcasting silently.

    numpy would happily broadcast a 7-wide action against the 6-wide constant
    tables in some shapes, producing plausible-looking garbage joint targets.
    """
    if array.shape[-1] != SO101_ACTION_DIM:
        raise ValueError(
            f"{name}: expected last axis {SO101_ACTION_DIM} "
            f"({', '.join(SO101_JOINT_NAMES)}), got shape {array.shape}"
        )
...
def rad_to_norm_torch(qpos):
    """Torch, on-device equivalent of :func:`rad_to_norm`.

    The observation path calls this every env step for every parallel env, so it
    must not round-trip joint positions through the host: ManiSkill keeps qpos on
    the GPU and the policy wants the state there too. Coefficients are cached per
    (device, dtype). Same idea as ``lerobot_state_scales_torch`` on the IsaacLab
    side of this work.
    """
    import torch

    _check_width(qpos, "rad_to_norm_torch")
    ...
    if cached is None:
        ...
        cached = (_t(OFFSET), _t(SIGN), _t(_RAW_OFFSET), _t(_RAW_SCALE))
        _TORCH_CONST_CACHE[key] = cached
    offset, sign, raw_offset, raw_scale = cached

    base = (qpos - offset) / sign
    tick = base / RAD_PER_TICK + CENTER_TICK
