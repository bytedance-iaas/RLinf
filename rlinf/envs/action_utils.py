...
def prepare_actions_for_maniskill(
    ...
    if "so100" in policy or "so101" in policy:
        ...
        # units (arm: [-100,100], gripper: [0,100]), NOT radians, while
        # ManiSkill's pd_joint_pos wants radians. Map via the follower
        # calibration; without this the arm slams into its joint limits.
    ...
    if "panda" in policy:
        # Panda EE-pose policies already emit env-ready actions
        # [num_envs, num_action_chunks, action_dim]. Pass through.
