# STATUS: ACTIVE — 当前流程在用。 阶段 G，协同训练数据集第二轮（留出 70-86），交付所用
...
OUT_REPO = "so101-cotrain-v15"
OUT_ROOT = "/data08/henryg/pai/data/so101-cotrain-v15"
TASK = "Grab the red cube"
# TRAIN on real episodes [0, TRAIN_EPISODES); hold the rest back so the offline
# sim2real check has real observations the model has never seen. The previous
# run put all 87 into training and then scored the check on episodes 0-3 of the
# same set -- a training-set number, not a generalisation one.
TRAIN_EPISODES = 70          # 0..69 train, 70..86 held out for evaluation
REPEAT = 3                   # 70 x 3 = 210 real vs 1292 sim -> ~23% of frames
...
def main():
    ...
    n_real_eps = min(TRAIN_EPISODES,
        json.load(open(os.path.join(REAL_ROOT, "meta", "info.json")))["total_episodes"])
    print(f"training on real episodes 0..{n_real_eps - 1}; the rest are held out", flush=True)
    added = 0
