...
def solve_grab_red_cube(env, seed=None, vis=False, xoff=0.0, yoff=0.0, drop_z=0.08, gjx=0.0, gjy=0.0):
    ...
    def fk_goto(txy, tz, steps=33, fine=False):
        """fine=True searches a NARROW window around the current pose at high
        resolution. The coarse global grid (0.25 rad in the elbow ~ several cm
        of TCP travel) cannot resolve a correction smaller than its own step,
        which is why the closed-loop refinement below used to leave a 2.4-7 cm
        drop error even though its stop threshold is 1.2 cm (measured over 298
        attempts, 2026-08-13)."""
        qs = robot.get_qpos()[0].cpu().numpy().copy()
        saved = qs.copy()
        best = None
        if fine:
            pan_r = np.linspace(qs[0] - 0.12, qs[0] + 0.12, 13)
            lift_r = np.linspace(qs[1] - 0.10, qs[1] + 0.10, 7)
            elb_r = np.linspace(qs[2] - 0.15, qs[2] + 0.15, 9)
            wf_r = np.linspace(qs[3] - 0.15, qs[3] + 0.15, 7)
            tol = 0.02
        else:
            pan_r = np.linspace(qs[0] - 0.7, qs[0] + 0.7, 15)
            lift_r = np.linspace(0.2, 1.4, 9)
            elb_r = np.linspace(-1.2, 0.8, 9)
            wf_r = np.linspace(-0.5, 1.2, 7)
            tol = 0.04
        for pan in pan_r:
            for lift in lift_r:
                for elb in elb_r:
    ...
    # 4 passes on the FINE grid, and the return value is checked -- the old
    # version silently no-opped whenever the coarse grid had no solution within
    # 4 cm, which is exactly what happens for far spawns (the hang vector
    # rotates with the pan swing, so the residual grows with spawn distance:
    # measured r=+0.51 between spawn y and drop error x).
    n_ref = 0
    for _ in range(4):
        if r2 == -1:
            break
        ...
        if np.linalg.norm(errv) < 0.010:
            break
        tcp_now = uenv.agent.tcp_pos[0].cpu().numpy()
        if not fk_goto(np.array([tcp_now[0], tcp_now[1]]) + errv, drop_z + 0.01,
                       steps=15, fine=True):
            print(f"  [dbg] refine {n_ref}: no fine solution for err="
                  f"{np.linalg.norm(errv):.3f}", flush=True)
            break
        n_ref += 1
    cube_now = uenv.red_cube.pose.sp.p
    print(f"  [dbg] pre-drop err="
          f"{np.linalg.norm(box_xy - np.array([float(cube_now[0]), float(cube_now[1])])):.4f}"
          f" after {n_ref} refinements", flush=True)
