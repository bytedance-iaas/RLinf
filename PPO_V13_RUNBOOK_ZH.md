...
**这是这个项目第一次让 PPO 真正放大策略。** 此前七次尝试全部失败或崩塌，失败清单见 §6。
...
| 诚实口径（从未用过的种子 4141/4242） | **58.6% / 57.0%** |
| 框内（legacy）参照 | **77.3%** |
| 全板参照 | **14.8%** |
| 耗时 | 约 2 小时（55 轮，每轮约 10 分钟） |

> **数字口径的注意事项**：73.4% 与 61.7% 是**同一套固定评测局面**上的前后对比，可以说明"涨了 11.7 点"；58.6/57.0% 是**全新种子**上的诚实值，两者不能混着比。要断言"PPO 在诚实口径下也涨了多少"，必须拿**同样的新种子**去测起点——这个对照实验单独在跑（`tools_so101_session/v13_baseline.sh`），结论出来前不要引用任何"诚实增益"数字。
...
> 连带纠正一条历史记载：保守配方里"把噪声减半"那一条**从来没有生效过**，pp4/v10 的实际噪声与失败的 v11 完全相同。
...
| 每轮更新次数 | 结果 |
|---|---|
| 12（官方配方） | step 4 从 61.7% 掉到 35.9%，step 9 掉到 **7.0%**，自动停机 |
| **1** | 稳定爬到 **73.4%** |
...
**推测的原因**：本任务每回合 64 次带噪决策，官方 ManiSkill 只有 16 次、LIBERO 48 次。同样的更新强度作用在更长时域、更脆的 BC 脊上。官方配方在它自己的基准上成立，**不能照搬到长时域任务**。
...
5% 这条线来自本项目四次运行的实测分界：pp4 5–9%、v10 10–15% 都放大成功；v6 0.5%、v11 1.0% 从未起来。**必要不充分**——v12 有 39% 仍然崩，因为更新次数不对。
...
v12 就是被第一条在 02:25 自动停掉的（step 9，7.0% vs 峰值 35.9%）。
...
| # | 配置 | 带噪 rollout | 结果 |
|---|---|---|---|
| 1 | 真机 SFT 起点直接 RL | — | 5 次运行、约 2000 轮、**零抓取** |
| 2 | 仿真 SFT 起点 + 默认参数 | — | 起点约 50%，**10 轮内塌到 0**，三次重现 |
| 3 | 冻结测试 `lr=1e-9` | — | 行为存活 66 轮 → 凶手是**更新步骤本身** |
| 4 | 手搓保守组（噪声"减半"、lr 2e-6、update_epoch 2、clip 0.1） | — | 46.9% → 75.0%（第 100 轮），随后衰减到 10.9%（第 320 轮） |
| 5 | 高起点（81.6%）再用手搓保守组 | — | **30 轮掉 53 点** |
