...
节点上的本地加速卡编号只统计该节点对外暴露的加速卡。如果节点在 Ray 启动前只开放了部分设备，例如容器中设置了 ``ASCEND_RT_VISIBLE_DEVICES=14,15``，RLinf 会认为该节点有两张加速卡，并在放置到本地编号 0 和 1 的 worker 的可见设备环境变量中写入设备 14 和 15，而不是 0 和 1。每个节点都按自身的设置转换，因此同一集群中的不同节点可以开放不同的设备子集。
...
见 :doc:`../reference/api/placement` 中对Placement策略的详细描述。
