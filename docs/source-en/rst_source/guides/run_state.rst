...
.. list-table::
   ...
   * - Verdict
     - Condition
   * - ``unreachable``
     - ``heartbeat_at`` is older than ``k × max(step_time_p50, floor)``.
       The process is gone.
   * - ``degraded``
...
Both the training side and any reader validate against the same committed
schema. Fixtures in ``tests/fixtures/run_state/`` are shared verbatim by both
sides so the two implementations cannot drift apart unnoticed.
