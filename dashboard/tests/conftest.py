...
The schema and the run fixtures are read from the repository root rather than
copied here. That is deliberate: they are the contract between the training side
and this one, and a copy is a copy that drifts. ``tests/unit_tests/`` on the
training side reads the same bytes, so a change that satisfies one side and
breaks the other cannot pass both suites.
"""
...
SCHEMA_PATH = os.path.join(REPO_ROOT, "docs", "schemas", "run.v2.schema.json")
FIXTURE_DIR = os.path.join(REPO_ROOT, "tests", "fixtures", "run_state")
...
def schema() -> dict:
    if not os.path.isfile(SCHEMA_PATH):
        pytest.skip(f"Shared schema not found at {SCHEMA_PATH}")
    return load_schema()
