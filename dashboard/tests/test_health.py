...
def test_progress_staleness_scales_with_the_run_s_own_step_time():
    """The same silence can be abnormal for a fast run and normal for a slow one."""
...
def test_liveness_does_not_scale_with_step_time():
    """A dead driver is detected on the same schedule for every step duration."""
...
def test_a_slower_configured_heartbeat_widens_only_the_liveness_budget():
    """A slower writer heartbeat widens only the process-liveness budget."""
    quiet = snapshot(
...
def test_verdict_carries_its_evidence():
    ...
    assert verdict.heartbeat_budget_s == 25.0
...
def test_every_bundled_fixture_classifies(name):
    """Each committed fixture yields a verdict from this implementation.

    Monorepo integration tests keep these fixtures byte-identical to the
    training-side copies. The dashboard cannot import ``rlinf``, so the contract
    is the binding.
    """
