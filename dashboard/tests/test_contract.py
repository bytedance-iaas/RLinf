...
"""Validate the dashboard reader against its bundled run-state contract.

So this file asserts three separate things:

1. every bundled fixture validates against the schema;
...
def test_fixture_validates_against_the_bundled_schema(schema, name):
    """Every reader fixture satisfies the bundled v2 contract."""
...
def test_reasoning_fixture_labels_its_step_axis_honestly():
    """Reasoning declares its minibatch step semantics.
...
def test_finished_fixture_has_a_checkpoint_with_resume_fields():
    """Resume metadata is structured instead of embedding a shell command.
