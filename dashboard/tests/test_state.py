...
def test_manifest_interval_controls_snapshot_and_file_liveness(run_tree, settings_for):
    """Both heartbeat signals use the interval recorded by the writer."""
    store, run = _store_and_run(
        run_tree,
        settings_for,
        manifest={"heartbeat_interval_s": 60.0},
        snapshot=_snapshot(heartbeat_at=_iso(NOW - timedelta(seconds=400))),
        heartbeat_seq=99,
    )
    heartbeat_path = os.path.join(run.run_root, "heartbeat")
    heartbeat_time = (NOW - timedelta(seconds=250)).timestamp()
    os.utime(heartbeat_path, (heartbeat_time, heartbeat_time))

    verdict = store.status(run, NOW).health
    assert verdict.health is Health.DEGRADED
    assert verdict.heartbeat_budget_s == 300.0
...
def test_only_completed_checkpoints_are_listed(run_tree, settings_for):
    """The checkpoint index contains completed saves only.
