...
期间通常已经能看到 run。Cluster、placement 或 worker-group launch 在
runner 构造前失败时，可能还没有任何 run 目录。若 `init_workers()` 失败，
通常只能看到 manifest，snapshot/health 为 unknown。
...
截至 2026-08-07，一个使用 PyTorch 2.11.0+cu130 的 LIBERO 验证镜像会在首次
rollout 时让 env 子进程退出，父进程在 `rlinf/envs/venv/venv.py` 收到
`EOFError`。Dashboard 的后端测试、server smoke 和 wheel smoke 均不受影响；
在该训练镜像兼容性问题解决前，不应把这个失败计入 Dashboard E2E 结果。
