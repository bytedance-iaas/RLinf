...
"""Query and stream recorded rollout media.
...
from .poster import VIDEO_SUFFIXES, PosterService
...
class MediaService:
    ...
    def __init__(self, store: StateStore, poster: PosterService | None = None):
        self._store = store
        self._poster = poster
    ...
    def list_media(
        ...
        *,
        ...
        # 404-ing all of them.
        posters = self._poster is not None and self._poster.available
        annotated = []
        for entry in entries:
            ...
            annotated.append(
                entry.model_copy(
                    update={
                        "path": path,
                        "url": self._url(run.run_id, path),
                        # Advertised only for the rows a poster can actually be
                        # made from. An image row is already its own preview, and
                        # a deployment with no ffmpeg says so by omission rather
                        # than by handing out URLs that 404.
                        "poster_url": (
                            self._poster_url(run.run_id, path)
                            if posters and path.lower().endswith(VIDEO_SUFFIXES)
                            else None
                        ),
                    }
                )
    ...
    @staticmethod
    def _poster_url(run_id: str, path: str) -> str:
        """Poster URL, stamped with the clip's size and mtime.

        The stamp is what makes the response cacheable for a day without risking
        a stale frame: rewrite the clip and the stamp changes, so the browser is
        asking for a different URL rather than revalidating an old one. A clip
        that cannot be stat-ed gets ``0-0``, which simply means it is never
        served from cache across a change -- the safe direction.
        """
        from urllib.parse import quote
        ...
        try:
            ...
            stamp = "0-0"
        return (
            f"/api/runs/{quote(run_id, safe='')}/media/poster"
            f"?path={quote(path)}&v={stamp}"
        )
