...
class StateStore:
    ...
    def _refine_with_heartbeat_file(
        self, run_root: str, verdict: HealthVerdict, now: datetime
    ) -> HealthVerdict:
        ...
        budget = verdict.budget_s or self._settings.timeout_floor_s
        if age <= self._settings.heartbeat_timeout_k * budget:
