...
These mirror the bundled ``schemas/run.v2.schema.json`` contract. The training
and dashboard sides keep separate models because the dashboard cannot import
``rlinf``. Monorepo integration tests keep the bundled contract byte-identical
to the producer's canonical copy.
...
class HealthVerdict(_Lenient):
    ...
    heartbeat_budget_s: float | None = None
...
class MediaEntry(_Lenient):
    ...
    url: str | None = None
    #: Server-relative URL for a single-frame poster, filled in by the API layer.
    #:
    #: ``None`` when this deployment cannot render one (no ffmpeg) or the row is
    #: already an image. The client uses its absence to decide *not to ask*,
    #: which is the difference between a quiet placeholder grid and forty 404s.
