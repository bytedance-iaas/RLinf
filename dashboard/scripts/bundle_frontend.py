...
``frontend/dist`` is a sibling of the ``rlinf_dashboard`` package, not part of
it, so ``pip wheel`` never saw it: an installed wheel served the API and no UI,
and said so only at debug level. This copies the build to
``rlinf_dashboard/static/``, which ``package-data`` does include.
...
def bundle() -> int:
    ...
    # earlier build is a file the new index.html does not reference, and it would
    # ship in the wheel forever.
    if TARGET.exists():
