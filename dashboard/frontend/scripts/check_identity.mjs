...
// Nothing in the type system or bundle notices, so this script provides a
// deterministic cross-run isolation check.
//
...
check(
  ...
  "run A's data is readable under run B's route -- cross-run payload leak",
);

console.log("the route changing empties the view before anything is fetched");
// Run A is loaded, the user selects run B, and B's first response has not
// arrived. Every field must already be gone, not just `data`: a timestamp or
// error carried across describes run B with run A's facts.
...
// Unguarded behavior: one `data` state with no identity, cleared only by an
// effect that has not run yet on the current render.
