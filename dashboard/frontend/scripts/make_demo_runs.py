...
#: falls back to synthesising one, and failing that to 64-byte stubs.
SAMPLE_CLIP: str | None = None

#: Whether to synthesise a playable clip when ``--sample-clip`` is not given.
#:
#: Stubs exercise the media view's error path but leave the grid unable to show
#: either playback *or* posters, so a developer checking those sees the same
#: nothing they would see from a bug. Synthesising costs one ffmpeg call.
SYNTHESISE_CLIPS: bool = True

#: Cache for the synthesised clip, so it is encoded once per invocation.
_SYNTHETIC_CLIP: str | None = None


def _synthetic_clip() -> str | None:
    """Encode one small playable MP4, or ``None`` if no encoder is available.

    Written the way the training side writes clips -- ``imageio``'s ffmpeg with
    no ``+faststart`` -- so the demo tree reproduces the trailing ``moov`` layout
    that the poster path exists to work around.
    """
    global _SYNTHETIC_CLIP
    if _SYNTHETIC_CLIP is not None:
        return _SYNTHETIC_CLIP

    try:
        sys.path.insert(0, os.path.join(os.path.dirname(__file__), "..", ".."))
        from rlinf_dashboard.poster import find_ffmpeg
    except Exception:  # noqa: BLE001 - the script must run outside the package too
        find_ffmpeg = None  # type: ignore[assignment]

    ffmpeg = find_ffmpeg() if find_ffmpeg is not None else shutil.which("ffmpeg")
    if not ffmpeg:
        return None

    handle, path = tempfile.mkstemp(prefix="rlinf-demo-clip-", suffix=".mp4")
    os.close(handle)
    try:
        subprocess.run(
            [
                ...
                "lavfi",
                "-i",
                "testsrc=size=512x512:rate=30:duration=3",
                "-pix_fmt",
                "yuv420p",
                "-y",
                path,
        ...
        return None
    _SYNTHETIC_CLIP = path
    return path


def _write_clip(path: str) -> None:
    source = SAMPLE_CLIP or (_synthetic_clip() if SYNTHESISE_CLIPS else None)
...
def touch_runs(root: str) -> int:
    ...
    writer's heartbeat interval, so generated healthy runs eventually read
    ``unreachable``. That is correct but makes a static fixture unsuitable for
    repeated UI checks.
    ...
    staleness offset. Using a liveness field as the reference would normalize the
    deliberately stale runs.
...
def make_run(
    ...
    *,
    ...
    # Per-worker logging stores the aggregate TensorBoard bundle under ``all/``.
...
def main() -> int:
    ...
    global SAMPLE_CLIP, SYNTHESISE_CLIPS
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Path to a real .mp4 to copy into every clip slot. Without it a small "
            "clip is synthesised with ffmpeg, which is enough for playback and "
            "posters but shows a test pattern rather than a rollout."
        ),
    )
    parser.add_argument(
        "--stub-clips",
        action="store_true",
        help=(
            "Write 64-byte stubs instead of playable clips. Exercises the media "
            "view's decode-failure path and the poster endpoint's 404."
        ),
    )
    parser.add_argument(
        "--ranks",
    ...
    if args.stub_clips:
        SYNTHESISE_CLIPS = False
        if args.sample_clip:
            parser.error("--stub-clips and --sample-clip are mutually exclusive")
    ...
    # A single point at x=1 exercises the zero-width float extent that requires
    # explicit padding before it reaches uPlot's axis-split loop.
    make_run(
        args.root,
