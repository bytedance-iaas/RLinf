 ...
 *
 * uPlot does not reject `min === max` for a single data point, and its axis-split
 * loop may choose an increment too small to advance in float64. Pad the
 * degenerate extent so every returned range remains numerically walkable.
 *
...
export function xExtent(xs: number[]): { min: number; max: number } | null {
  ...
  // large that 1 falls below its float spacing. Inputs still come from disk and
  // must produce a safe range.
 ...
 *
 * Live updates use `setData(data, false)` to preserve stable axes. This helper
 * expands the y range when a new extreme would otherwise fall outside it.
 *
...
export function yGrowth(
  ...
  // Match the initial range padding so live growth and a fresh render agree.
