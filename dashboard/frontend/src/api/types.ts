...
export interface MediaEntry {
  ...
  url: string | null;
  /**
   * Single-frame preview, or `null` when the server cannot render one (no
   * ffmpeg, or the row is already an image).
   *
   * Absence is the instruction not to ask: a deployment without ffmpeg omits
   * this rather than handing over a URL that 404s once per card.
   */
