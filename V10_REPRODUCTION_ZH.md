...
| 参数 | 值 | 依据 |
|---|---|---|
| `SO101_COLLECT_DIR` | 目录 | 打开 `ManiskillEnv` 的录制器：每路环境**首次成功**时把 (前视, 腕视, 状态, 动作) 落盘为 `.npz`，分路重置安全 |
...
| 奖励阶梯无刷分路径 | 手算每个状态的**每步**收益（见 F.2） | 改奖励，别指望 PPO 绕过去 |
| 批量算术自洽 | `global_batch = num_envs × (预算/动作块长) × rollout_epoch ÷ 每轮更新数` | 见 F.3 |

### F.1 代码接线（本仓库已全部就位，无需改动）
...
### F.3 配置文件：`examples/embodiment/config/so101_ppo_v6_official.yaml`
...
| `env.train.ignore_terminations` | **True** | **[TASK]** 见 F.2 的终止陷阱 |
...
## 阶段 G —— PPO 在线微调（**已跑通，2026-08-14**）

> 阶段 F 记录的是"怎么接线"。这一节记录的是**实际跑通的那次**：起点、每个改动的**实测依据**、完整命令、以及为什么其它配置会失败。
> 结果：环 1 上 eval **61.7% → 73.4%**（峰值 @ step 29），检查点 `so101_ppo_v13/.../global_step_30`。

### G.0 三个决定成败的参数（其余抄官方即可）
