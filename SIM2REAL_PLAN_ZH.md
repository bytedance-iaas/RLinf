...
> ⚠️ **但这条路线的等价性验证目前没过（§4.3），所以现在请走 §4.10 的路线 B。** 本节保留，因为差距已经缩小、原因也定位到了一半，修好之后它仍是更好的部署方式。
...
> ### ⚠️ 实测结果：**这道门没过，路线 A 暂时不可用**
>
> | 次序 | 改动 | 留出集比值 |
> |---|---|---|
> | 首次导出 | —— | **0.88** ❌ |
> | 修正去噪步数 10 → 4 | LeRobot `num_inference_steps` 默认 10，而全流程是 4 | **0.83** ❌ |
> | 判据 | | ≤0.80 才算等价 |
>
> **逐关节看，劣化是均匀的 ~18%，不是某一两维**；而且 hold-still 那一列两边逐个相同，**说明数据路径完全一致，唯一变量就是策略本身**。
>
> 已排除：数据路径、某维统计量、图像分辨率（两边都是 224 + resize_with_pad）、归一化公式（两边都是 `2(x−q01)/(q99−q01)−1`）、dtype（LeRobot 上转 fp32，无损，解释不了 18%）。
> 仍未排除：状态补零到 32 维的顺序（归一化前还是后）、离散状态 token 的细节、图像 resize 的插值差异。
>
> **这正是这道门存在的意义**：813 个键全部对上、加载零报错、`All keys loaded successfully`——**看起来完全正常，行为却差 18%**。只比对键集就会把一个未验证的策略放上机械臂。
